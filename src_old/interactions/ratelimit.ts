...
}, { coolTime: Duration.toMS('5s') }, async (interaction) => {

  if (!interaction.inCachedGuild() || !interaction.channel) return;
  const duration = interaction.options.getInteger('duration', true);

  if (!interaction.appPermissions?.has(PermissionFlagsBits.ManageChannels)) return interaction.reply({ content: '`❌` BOTの権限が不足しているため、低速モードを変更できませんでした。', ephemeral: true });
...
export default [setRateLimitCommand];