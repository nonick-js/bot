...
const addRoleSelectButton = new Button({
  ...
  if (interaction.message.components[0].components[0].type !== ComponentType.StringSelect) return interaction.reply({ content: '`❌` セレクトメニューを作成していません！', ephemeral: true });
  ...
  if (targetMessage.components.length === 5) return interaction.reply({ content: '`❌` これ以上コンポーネントを追加できません！', ephemeral: true });
  if (targetMessage.components[0]?.components[0]?.type === ComponentType.Button) return interaction.reply({ content: '`❌` セレクトメニューとボタンは同じメッセージに追加できません。', ephemeral: true });
...
export default [addRoleSelectButton];