...
const generalSetting = [
  ...
  new Modal({
    ...
    if (isNaN(start) || start > 23 || start < 0) return interaction.reply({ content: '`❌` 開始時間に無効な値が入力されました。', ephemeral: true });
    if (isNaN(end) || end > 23 || end < 0) return interaction.reply({ content: '`❌` 終了時間に無効な値が入力されました。', ephemeral: true });
    if (start === end) return interaction.reply({ content: '`❌` 開始時間と終了時間を同じ値に設定することはできません。', ephemeral: true });
  ...
  // 認証レベル
  new SelectMenu({
    customId: 'nonick-js:setting-changeVerificationLevel-level',
...
const logSetting = [
  ...
  new Button({
    ...
    interaction.showModal(channelModal.setCustomId('nonick-js:setting-changeVerificationLevel-log-channel-modal'));
  }),
  new Modal({
    customId: 'nonick-js:setting-changeVerificationLevel-log-channel-modal',
  }, (interaction) => {
    changeChannelSetting(interaction, 'changeVerificationLevel.log.channel', FeatureType.ChangeVerificationLevel);
  }),
];

export default [...generalSetting, ...logSetting];