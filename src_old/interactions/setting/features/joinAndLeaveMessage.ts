...
const joinMessageSetting = [
  ...
  new Button({
    ...
    interaction.showModal(channelModal.setCustomId('nonick-js:setting-message-join-channel-modal'));
  }),
  new Modal({
    customId: 'nonick-js:setting-message-join-channel-modal',
  }, async (interaction) => {
    changeChannelSetting(interaction, 'message.join.channel', FeatureType.JoinAndLeaveMessage);
  }),
  ...
  new Modal({
    ...
    if (!title && !description) return interaction.reply({ content: '`❌` タイトルと説明はどちらかは必ず入力する必要があります。', ephemeral: true });
...
const leaveMessageSetting = [
  ...
  new Button({
    ...
    interaction.showModal(channelModal.setCustomId('nonick-js:setting-message-leave-channel-modal'));
  }),
  new Modal({
    customId: 'nonick-js:setting-message-leave-channel-modal',
  }, (interaction) => {
    changeChannelSetting(interaction, 'message.leave.channel', FeatureType.JoinAndLeaveMessage);
  }),
...
export default [...joinMessageSetting, ...leaveMessageSetting];