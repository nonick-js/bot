...
const LogConfig = z.discriminatedUnion('enabled', [
  z.object({
...
// 入室メッセージ
export const JoinMessageConfig = baseSchema.extend({
  enabled:   z.boolean(),
  channel:   Snowflake.nullable(),
...
// 退室メッセージ
export const LeaveMessageConfig = baseSchema.extend({
  enabled:   z.boolean(),
  channel:   Snowflake.nullable(),
  message:   MessageOption,
  ignoreBot: z.boolean(),
...
// サーバー内通報
export const ReportConfig = baseSchema.extend({
  channel:          Snowflake,
...
export const MessageExpandConfig = baseSchema.extend({
  ...
  ignorePrefixes:     z.array(z.string().length(1, 'プレフィックスは1文字である必要があります')).max(5, '5個以上のプレフィックスを登録することはできません。'),
});
...
// 自動認証レベル変更
export const AutoChangeVerifyLevelConfig = baseSchema.extend({
  enabled:    z.boolean(),
...
// AutoMod Plus
export const AutoModConfig = baseSchema.extend({
  enabled:          z.boolean(),
  domainFilter:     z.boolean(),
  domainFilterList: z.array(z.string().regex(/^((?!-)[A-Za-z0-9-]{1,63}(?<!-)\.)+[A-Za-z]{2,6}$/, '無効なドメインです。')).max(20, '20個以上のドメインを登録することはできません。'),
