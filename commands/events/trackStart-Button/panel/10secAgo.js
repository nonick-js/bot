...
module.exports = {
    ...
    data: { customid: 'music-10secAgo', type: 'BUTTON' },
    ...
    exec: async (client, interaction, Configs, language, player) => {
        ...
        const time = queue.streamTime - 10000 < 0 ? 0 : queue.streamTime;
        await queue.seek(time);
        interaction.update({ components: [button], ephemeral: true });
        // eslint-disable-next-line no-empty-function
        await queue.metadata.channel.send(language('MUSIC_10SECAGO_SUCCESS')).catch(() => {});
    },
};