...
const modalInteraction = {
  data: {
	...
	exec: async (interaction) => {
		...
		const { reportCh, reportRole, reportRoleMention } = basicModel.get();
		...
		if (!channel) {
			await basicModel.update({ reportCh: null });
		...
		channel.send({ content: reportRoleMention ? `${discord.roleMention(reportRole)}\n${content}` : content, embeds: [contentEmbed], components: [button] })
			.then(() => {
