      ...
      const color = interaction.options.getString('color');
		...
		if (interaction.options.getSubcommand() == 'import') {
			...
			interaction.followUp({ content: '**プレビューモード**\n「送信」ボタンを押すとこのチャンネルに埋め込みを送信します。', embeds: [json.data], components: [button1, button2], ephemeral: true })
				.catch(() => {
					const error = new discord.EmbedBuilder()
						.setAuthor({ name: 'ファイルからの埋め込みの生成に失敗しました！', iconURL: 'https://cdn.discordapp.com/attachments/958791423161954445/1022819275456651294/mark_batsu_illust_899.png' })
						.setDescription('[埋め込みの制限](https://discordjs.guide/popular-topics/embeds.html#embed-limits)に違反していないか、有効なファイルであるか確認してください。')
						.setColor('Red');
