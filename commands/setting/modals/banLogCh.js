...
module.exports = {
    ...
    data: { customid: 'modal-setting-banLogCh', type: 'MODAL' },
    /** @type {InteractionCallback} */
    exec: async (interaction, client, Configs) => {
        const config = await Configs.findOne({ where: { serverId: interaction.guild.id } });
        const welcome = config.get('banLog');
        ...
        try {
            ...
            const successembed = new discord.MessageEmbed()
                .setDescription('✅ BANコマンドの使用ログがここに送信されます!')
                .setColor('GREEN');
            channel.send({ embeds: [successembed] })
                .then(() => {
                    Configs.update({ banLogCh: channel.id }, { where: { serverId: interaction.guildId } });
                    if (welcome) {embed.spliceFields(0, 1, { name: 'ログ機能', value: `${discord.Formatters.formatEmoji('758380151544217670')}有効 (${discord.Formatters.channelMention(channel.id)})`, inline:true });}
