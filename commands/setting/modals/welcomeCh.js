...
module.exports = {
    ...
    data: { customid: 'modal-setting-welcomeCh', type: 'MODAL' },
    ...
    exec: async (interaction, client, Configs) => {
        ...
        try {
            ...
            channel.send({ embeds: [successembed] })
                .then(() => {
                    Configs.update({ welcomeCh: channel.id }, { where: { serverId: interaction.guildId } });
                    if (welcome) {embed.spliceFields(0, 1, { name: '入室ログ', value: `${discord.Formatters.formatEmoji('758380151544217670')}有効 (${discord.Formatters.channelMention(channel.id)})`, inline:true });}
