export class PlaceHolder<T extends Readonly<Record<string, unknown>>> {
  private holder: Map<
    string,
    { key: string; callback: (params: Partial<T>) => unknown }
  >;
  ...
  register(
    key: string,
    callback: (params: Partial<T>) => unknown,
    force = false,
  ) {
...
export namespace Duration {
  ...
  export function toMS(text: string) {
    ...
    return Object.entries(match?.groups ?? {}).reduce(
      (p, [key, value = 0]) =>
        p + Number(value) * durations[key as List].time || 0,
      0,
    );
  }
  ...
  export function format(
    ...
    return Object.entries(parse(ms, pass))
      .map(([short, duration]) => {
        ...
        return `${Math.sign(ms) === -1 ? '-' : ''}${duration}${
          template ? short : long
        }`;
      })
      .join(' ');
