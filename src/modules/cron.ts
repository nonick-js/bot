...
import { ScheduleOptions, schedule, validate, getTasks } from 'node-cron';
...
export class Cron {
  ...
  static setTimeZone(zone: string) {
    this.instance.options.timezone = zone;
    return this;
  }

  static register(cronExpression: string | Cron.Schedule, func: (now: Date | 'manual' | 'init') => void, options?: ScheduleOptions) {
  ...
  static async registerFiles(basePath: string, predicate?: (value: fs.Dirent) => boolean) {
    for (const filePath of Cron.getAllPath(basePath, predicate)) {
      ...
      for (const { cronExpression, func, options } of schedules) Cron.register(cronExpression, func, options);
    }
  }

  private static getAllPath(basePath: string, predicate?: (value: fs.Dirent) => boolean, pre = new Set<string>()) {
    ...
    fs.readdirSync(basePath, { withFileTypes: true }).forEach((v) => {
      ...
      if (v.isDirectory() && (predicate?.(v) ?? true)) Cron.getAllPath(path.resolve(basePath, v.name), predicate, pre);
    });
    return [...pre];
  }

  private static parseExpressions(expression: Cron.Schedule) {
    const exp: Required<Cron.Schedule> = { second: 0, minute: '*', hour: '*', date: '*', month: '*', day: '*', ...expression };
    const parsed = Object.entries(exp).map(([k, v]) => Cron.parseExpression(v, Cron.exp[k as keyof typeof Cron.exp]));
    return parsed.join(' ');
  }
...
export namespace Cron {
  export type Schedule = Partial<Record<
    | 'second'
    | 'minute'
    | 'hour'
    | 'date'
...
function* range(min: number, max: number = 0) {
