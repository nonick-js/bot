...
const verifyCommand = new ChatInput({
  ...
  dmPermission: false,
}, { coolTime: Duration.toMS('10m') }, async (interaction) => {
  ...
  if (role.managed || role.id === interaction.guild.roles.everyone.id) return interaction.reply({ content: '`❌` そのロールは認証に使用することはできません', ephemeral: true });
  if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator) && interaction.member.roles.highest.position < role.position) return interaction.reply({ content: '`❌` 自分の持つロールより上のロールを認証に使用することはできません', ephemeral: true });
  if (!role.editable) return interaction.reply({ content: '`❌` そのロールはBOTより高い位置にあるため、認証に使用することはできません', ephemeral: true });
...
const verifyButton = new Button({
  ...
  if (duringAuthentication.has(interaction.user.id)) return interaction.reply({ content: '`❌` 現在別の認証を行っています。認証が終了するまで新たな認証を行うことはできません。', ephemeral: true });
  if (!roleId || !(roles instanceof GuildMemberRoleManager)) return interaction.reply({ content: '`❌` 認証中に問題が発生しました。', ephemeral: true });
  if (roles.cache.has(roleId)) return interaction.reply({ content: '`✅` 既に認証されています。', ephemeral: true });
