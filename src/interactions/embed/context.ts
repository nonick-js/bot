...
const context = new MessageContext({
  ...
  if (!interaction.appPermissions?.has(PermissionFlagsBits.ManageWebhooks)) return interaction.reply({ content: '`❌` この機能を使用するにはBOTに`ウェブフックの管理`権限を付与する必要があります。', ephemeral: true });

  const webhook = await interaction.targetMessage.fetchWebhook().catch(() => null);
  if (!webhook || !interaction.client.user.equals(webhook.owner as User)) return interaction.reply({ content: '`❌` NoNICK.jsを使用し、かつ現在も有効なWebhookで投稿した埋め込みのみ編集できます。', ephemeral: true });
...
const select = new SelectMenu({
  ...
  if (!targetMessage) return interaction.update({ content: '`❌` メッセージの取得中に問題が発生しました。', embeds: [], components: [] });
  ...
  if (interaction.values[0] === 'editEmbed') {
    ...
    if (!interaction.member.permissions.has(PermissionFlagsBits.ManageRoles)) return interaction.reply({ content: '`❌` あなたの権限ではこの機能は使用できません。', ephemeral: true });
    ...
    if (targetMessage.components.length === 0) return interaction.reply({ content: '`❌` コンポーネントを一つも追加していません。', ephemeral: true });
