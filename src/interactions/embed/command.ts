...
const command = new ChatInput({
  ...
  if (!interaction.channel?.isTextBased()) return interaction.reply({ content: '`❌` このチャンネルでは使用できません', ephemeral: true });
  ...
  if (subCommand === 'create') {
    ...
    if (!title && !description) return interaction.reply({ content: '`❌` `title`と`description`はどちらかは必ず入力する必要があります。', ephemeral: true });
    ...
    if (!attachment.contentType?.startsWith('application/json')) return interaction.reply({ content: '`❌` 添付されたファイルはjsonファイルではありません。', ephemeral: true });
    if (attachment.size > 3000000) return interaction.reply({ content: '`❌` 3MB以上のjsonファイルはインポートできません。', ephemeral: true });
    ...
    if (!interaction.appPermissions?.has(PermissionFlagsBits.ManageWebhooks)) return interaction.reply({ content: '`❌` この機能を使用するには、NoNICK.jsに`ウェブフックの管理`権限を付与する必要があります。', ephemeral: true });
    if (avatar instanceof Attachment && (!avatar.contentType || !['image/png', 'image/jpeg'].includes(avatar.contentType))) return interaction.reply({ content: '`❌` アバター画像には`jpeg`または`png`のみ使用できます。', ephemeral: true });
    ...
    else { interaction.followUp({ content: '`❌` プロフィールの変更に失敗しました。', ephemeral: true }); }
  }
});
