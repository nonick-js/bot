...
const sendLinkButtonModal = new Modal({
  ...
  const emoji = interaction.guild.emojis.cache.find(v => v.name === emojiNameOrId)?.id || emojiNameOrId.match(RegEx.Emoji)?.[0];

  if (!label && !emoji) return interaction.reply({ content: '`❌` ボタンのテキストと絵文字はどちらは必ず入力する必要があります', ephemeral: true });
  if (!isURL(url)) return interaction.reply({ content: '`❌` `http://`または`https://`から始まるURLを入力してください。', ephemeral: true });
...
export default [sendLinkButton, sendLinkButtonModal];