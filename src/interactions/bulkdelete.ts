...
}, { coolTime: Duration.toMS('10s') }, async (interaction) => {

  if (!interaction.inCachedGuild() || !interaction.channel) return;

  if (!interaction.appPermissions?.has(PermissionFlagsBits.ManageMessages)) return interaction.reply({ content: '`❌` BOTの権限が不足しているため、メッセージを削除できませんでした。', ephemeral: true });
...
export default [bulkDeleteMessagesCommand];
