...
const setTimeStampModal = new Modal(
  ...
  (interaction): void => {
    ...
    if (timeStamp !== '' && !/^\d{4}-?\d\d-?\d\d(?:T\d\d(?::?\d\d(?::?\d\d(?:\.\d+)?)?)?(?:Z|[+-]\d\d:?\d\d)?)?$/.test(timeStamp)) {
      interaction.reply({ content: '`❌` 有効なタイムスタンプではありません！[ISO8601](https://ja.wikipedia.org/wiki/ISO_8601)に準拠した値を入力してください。', ephemeral: true });
      return;
    }
    ...
    interaction
      .update({ embeds: [
        EmbedBuilder
          ...
          .setTimestamp(timeStamp !== null ? new Date(timeStamp).getTime() : null),
      ] })
      .catch(() => {
