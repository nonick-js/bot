...
const setBasicTextsModal = new Modal(
  ...
  (interaction): void => {
    ...
    if (!title && !description) {
      interaction.reply({ content: '`❌` タイトルと説明はどちらかは必ず入力する必要があります。', ephemeral: true });
      return;
    }
    ...
    try {
      ...
      interaction.reply({ content: '`❌` 無効なカラーコード、または色の名前が入力されました。[このページ](https://docs.nonick-js.com/nonick.js/features/embed/)を参考に正しい値を入力してください。', ephemeral: true });
      return;
    }
