...
import { embedCreateButtons } from './embed/_components';
  ...
  async (interaction): Promise<void> => {
    ...
    if (subCommand == 'create') {
      ...
      if (!title && !description) {
        interaction.reply({ content: '`❌` `title`と`description`はどちらかは必ず入力する必要があります。', ephemeral: true });
        return;
      }
    ...
    if (subCommand == 'import') {
      ...
      if (!attachment.contentType?.includes('application/json')) {
        interaction.reply({ content: '`❌` 添付されたファイルがjsonファイルではありません。', ephemeral: true });
        return;
      }
      else if (attachment.size > 3000000) {
        interaction.reply({ content: '`❌` 3MB以上のjsonファイルはインポートできません。', ephemeral: true });
        return;
      }
